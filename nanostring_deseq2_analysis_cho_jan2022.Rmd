...
Let's test again looking at just obese vs lean (defined as > 30 BMI and < 25 BMI). In this way of subsetting the data, there are:

`r table(dat$status_ob)`
...
After correcting for sample batch using the "RUVg" method of Love, et. al., we found X miRNAs that were DE (p-adj < 0.1) between overweight/obese and lean samples.  We also found Y miRNAs that were DE (p-adj < 0.1) between obese and lean samples.  75% of the overweight DE miRNAs were also captured in the obese comparison, despite having fewer samples in the analysis.  
