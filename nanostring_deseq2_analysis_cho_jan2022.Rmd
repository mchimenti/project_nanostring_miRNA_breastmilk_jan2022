...
## DE analysis subsetting by batch 

From our exploratory analysis, it seems that batches 5-9 are the most homogenous w.r.t each other.  Batch 4 is a definite outlier, and 1-3 are different in important ways as well, including being biased towards later collection dates.  

While I tried interrogating just batches 5-9 for DE miRNA, unfortunately there were no results.  This may be owing to a reduction in power or for some other reason.  I am not sure.  

So here we will just drop batch 4 (4-1 and 4-2) altogether. 



```{r, batch_subset, class.source = 'fold-show'}

dat <- norm.dat.k2$set
dat_batch <- dat[,dat$batch %in% c("1","2","3","5","6","7","8","9")]
pData_batch <- pData(dat_batch)
pData_batch <- pData_batch %>% as_tibble() %>% mutate(milk_col_factor = as.factor(ntile(milk_col_dt, 4))) #Quartile bins of collection date

dds_batch <- DESeqDataSetFromMatrix(countData = counts(dat_batch[26:828,]),
                              colData = pData_batch,
                              design = ~ W_1 + W_2 + status_ob)  
...
res_OB_drop4 <- as.data.frame(results(dds_batch,contrast = c('status_ob','Obese','Lean')))
```
...
res_OB_drop4_sig
```
There are `r dim(res_OB_drop4_sig)[1]` DE miRNAs when dropping batch 4 from the analysis.  

```{r}
v <- Vennerable::Venn(list(obese_milk_factor = rownames(res_OB_MCF_sig), obese_drop_batch4 = rownames(res_OB_drop4_sig)))
plot(v, doWeights = TRUE)
```

These are the 8 in common between the full analysis and the drop batch 4 analysis: 

```{r}
v@IntersectionSets$`11`
```

## DE analysis milk collection day < 40 

In this section, we will look for DE miRNAs using only those samples collected less than or equal to 40 days post-natality.  We will compare obese to normal.  

```{r, milk_dat_subset, class.source = 'fold-show'}

dat <- norm.dat.k2$set
dat_lt40 <- dat[,dat$milk_col_dt <= 40]
pData_lt40 <- pData(dat_lt40)


dds_lt40 <- DESeqDataSetFromMatrix(countData = counts(dat_lt40[26:828,]),
                              colData = pData_lt40,
                              design = ~ W_1 + W_2 + status_ob)  
dds_lt40 <- DESeq(dds_lt40, fit='local')


```
```{r}

res_OB_lt40 <- as.data.frame(results(dds_lt40,contrast = c('status_ob','Obese','Lean')))
```
...
Unfortunately, there are only two miRNAs detected this way, with 575|0 being DE owing to one outlier sample.  On the other hand, 205-5p|0 does look promising and was also detected in the "drop batch4" analysis above.  

```{r}
df <- plotCounts(dds_lt40, gene = "hsa-miR-575|0", intgroup = "status_ob", returnData = TRUE)
p <- ggboxplot(df, 
               x='status_ob', 
               y='count', 
               fill = "status_ob",
...
df <- plotCounts(dds_lt40, gene = "hsa-miR-205-5p|0", intgroup = "status_ob", returnData = TRUE)
p <- ggboxplot(df, 
               x='status_ob', 
               y='count', 
               fill = "status_ob",
...
p <- p + ggtitle("hsa-miR-205-5p|0 Counts by Status")
p

```

## DE analysis collection date < 40

Feb 22, 2022 request: 

Please compare the miRNA profiles between the group of sample collection date <40 and the group of the rest. We may need to exclude batch 4s, and control the BMI before comparing these groups.
...
```{r}
df <- plotCounts(dds_drop4_day40, gene = "hsa-miR-27b-3p|0", intgroup = c("status_ov","milk_col_factor"), returnData = TRUE)
p <- ggboxplot(df, 
               x='milk_col_factor', 
               y='count', 
               #fill = "status_ov",
...
p <- p + ggtitle("hsa-miR-27b-3p|0 Counts by Status")
p
...
**In this analysis, we aim to determine differentially expressed bEV-miRNAs in obese mothers;  EV-miRNA expression was profiled with Nanostring Human NanoString miRNA Expression Panel of 798 biologically relevant miRNAs. Our working hypothesis is that immune-response and metabolism-related miRNAs are altered in overweight/obese mother breast milk.**

**After correcting for sample batch using the "RUVg" method of Love, et. al., we found `r dim(res_OV_sig)[1]` miRNAs that were DE (p-adj < 0.1) between overweight/obese and lean samples.  We also found `r dim(res_OB_sig)[1]` miRNAs that were DE (p-adj < 0.1) between obese and lean samples.  22 of the overweight DE miRNAs were also captured in the obese comparison, despite having fewer samples in the analysis.**  

**Many of the most DE miRNAs have significant outlier samples that may be biasing the results.  We may wish to drop one or more samples, or possibly the most outlier batch (probably batch 4) and re-evaluate. **

**When dropping batch 4 from the analysis, we obtain fewer DE miRNAs.  There are still 8 that overlap the analysis for Obese vs Lean for the full dataset.  I would possibly consider prioritizing these 8 for follow up wet lab studies to validate. **

**In future studies, we may wish to put more emphasis on controlling the technical factors in sample preparation for sequencing.  There appear to be large batch effects that are present and likely masking some of the biologically interesting signal.  Additionally, factors like "milk collection date" should be randomized across batches or standardized to one particular interval (i.e., 12 days).**
